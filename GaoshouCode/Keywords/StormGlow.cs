...
// 打出会先扣能量、再扣辉星（游戏扣费顺序）；高手护符（初始遗物）会在"消耗能量/辉星"事件里互抵补回。
// 规则（事件制，每回合 3 次）：每次"消耗能量"事件 +1 辉星；每次"消耗辉星"事件 +1 能量（与消耗量无关）。
// 模拟顺序（与伪代码一致）：
//   1) 若持护符且有能量费：预测补 1 辉星，护符次数 -1（先补辉星方向）。
//   2) 若持护符（剩余次数被上一步递减后仍有）且有辉星费：预测补 1 能量。
//   3) 风暴就绪 = 打出后辉星 >= StarsStorm 且 打出后能量 >= EnergyStorm。
...
public static class StormGlow
{
    public static bool Ready(CardModel card, int needEnergy, int needStars)
    {
        ...
        // CanonicalStarCost 默认 = -1 表示"无辉星费"；clamp 到 0 避免被当作负费用（造成 s 凭空 +1）。
        ...
        if (amulet != null)
        {
            ...
            // 先能量后辉星的扣费顺序 → 先检查"能量消耗"是否补辉星。
            if (costE > 0 && uses > 0)
            {
                s += 1;   // 消耗能量 → 补 1 辉星
                uses--;
            }
            // 再检查"辉星消耗"是否补能量（用的是递减后次数，阻止单次护符被双方向消耗）。
            if (costS > 0 && uses > 0)
            {
                e += 1;   // 消耗辉星 → 补 1 能量
            }
